...
startprg
		...
		JSR checkip		; display the IP
		LDA chksm		; get checksum of IP 
		CMP #$4A		; chksum for "0.0.0.0"
		BEQ noip
		JMP portal		; valid IP? -> go to portal

noip	LDX #4			; X+1 rows down
		LDY #$00		; y+1 cols right
		JSR $E50C		; cursor pos X Y
		LDA #<text1
		LDY #>text1
...
text0	
		...
		!by 31,224,156,225,159,226,227,228,158,229,230,31,231,"w","I","C","2","0"," ","l","A","U","N","C","H","E","R"
		!by 31,235,236,236,236,236,236,236,237,32,32,32,32,32,32,32,32,32,32,32,32,32,239,146
		!by 30,$00

text0a
		!by 30
		!text " pLEASE wAIT ...",$0D,$0D
		!by 0
...
checkip						; send "get IP" to WiC and check for valid IP
		LDA #$00
		STA chksm			; clear checksum of IP
		LDA com_jmp+3 
		TAY 
		LDA com_jmp+2 
		JSR wiccmd
...
-		JSR read_byte		; fetch data from WiC
		...
		BNE ++				; branch to print
+		LDA #$20			; print SPACE
++		NOP 				; A contains data send from WiC64
		CLC 
		ADC chksm			; build checksum of IP
		STA chksm
		DEX
		BNE -
		DEY
